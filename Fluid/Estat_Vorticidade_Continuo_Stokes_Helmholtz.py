...
pasta = "Estat2_flinear_rmin"+str(args.r)+"_v_"+str(args.v)+"/"
...
class ObjR(ROL.Objective):
    ...
    def update(self, x, flag, iteration):
        ...
        if iteration >= 0:
            ...
            self.contador += 1
            print("O contator : %s" % self.contador)
            """if self.contador in [3, 30, 55, 80]:
                #Gravando
                #nova_malha = nova_malha(mesh, rho_o, 0.55, None)
                hdf5 = HDF5File(mesh.mpi_comm(), pasta+"Resultado"+str(self.contador)+".h5", "w")
                hdf5.write(mesh, "mesh")
                hdf5.write(rho_o, "solucao")"""
