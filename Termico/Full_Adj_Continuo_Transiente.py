...
pasta = "continuo_transiente_full/"
...
def prob_direto(m, t, t0):
    ...
    return (F, bc1)
def prob_direto2(m, t, t0):
    v = TestFunction(t.function_space())
    F2 = - inner(k(m)*grad(t), grad(v))*dx
    bc2 = DirichletBC(T, 273., "on_boundary")
    return (F2, bc2)
...
def solve_adjoint(m, t, t0):
    listADJ = []
    for i in range(1,len(t)):
        (F2, bc2) = prob_direto2(m, t[i], t0[i])
        bc2.homogenize() # adjoint has homogeneous BCs
        J = functional(m, t[i])
        adj = Function(T)
        adj_tst = TestFunction(T)
        solve(-k(m)*inner(grad(adj), grad(adj_tst))*dx - 2*inner(t[i], adj_tst)*dx==0, adj, [bc2])
        """Se quiser desencanar do desenvolvimento manual anterior, só descomentar
        estas linhas
        adF = adjoint(derivative(F2, t[i]))
        dJ = derivative(J, t[i], TestFunction(t[i].function_space()))
        solve(action(adF, adj) - dJ == 0, adj, [bc2])"""
        listADJ.append(adj)
    return listADJ
...
#t = Function(T)
t, memoT, memoT0 = resp_direto(m,"problema_direto_inicial")
...
class ObjR(ROL.Objective):
    ...
    def value(self, x, tol):
        ...
        for i in range(1, len(self.t)):
            J = functional(self.m, self.t[i-1])
            mJ += assemble(J)
        return mJ
    ...
    def gradient(self, g, x, tol):
        ...
        t = self.t
        t0 = self.t0
        lam = solve_adjoint(m, t, t0)
        dmo = TestFunction(A)
        L = 0
        for i in range(1,len(t)):
            L += dt * beta*(2*m-(6*m**2)+(4*m**3)) * dmo * dx + kdash(m) * dmo * inner(grad(t[i-1]), grad(lam[i-1]) ) * dx #- 1./dt * kdash(m) * dmo * inner(t[i-1], lam[i-1]) * ds #inner(k(m)*grad(t), grad(v))*dx
        deriv = assemble(L)
    ...
    def update(self, x, flag, iteration):
        ...
        #self.t.assign(listaT)
        #self.t0.assign(listaT0)
        self.t = listaT
        self.t0 = listaT0
        if iteration >= 0:
            control_file << self.m
            state_file << self.t[len(self.t)-1]
