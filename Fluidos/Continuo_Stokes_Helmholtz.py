...
pasta = "stokes_continuo_helmholtz/"
...
def prob_direto(rho, w):
    ...
    (u, p) = split(w)
    (v, q) = TestFunctions(W)
    rho = density_filter(rho, r_min) #Filtro Helmholtz
...
def adjunto(rho, w, J):
    ...
    n = FacetNormal(mesh)
    df2dmt = r_min**2*inner(grad(adj2),grad(adj2_tst))*dx + inner(adj2, adj2_tst)*dx -r_min**2*inner(n,grad(adj2))*adj2_tst*ds
...
class ObjR(ROL.Objective):
    ...
    def gradient(self, g, x, tol):
        ...
        L = 0.5 * alphadash(rho_o) * drho * inner(u, u) * dx + drho * lam * dx
        deriv = assemble(L)
