...
def adjunto(rho, w, J):
    ...
    F_ad = ( mu * inner(grad(u_ad), grad(v_ad)) +
        alpha(rho)*inner(u_ad, v_ad) -
        inner(grad(p_ad), v_ad) +
        q_ad*div(u_ad) ) * dx #- mu*inner(grad(v_ad)*n, u_ad)*ds

    w_resp = resp_direto(rho)
    u_resp, p_resp = split(w_resp)

    dJdu = (2 * mu * inner(grad(u_resp),grad(v_ad)) + #colocando - aqui da certo
        alpha(rho) * inner(u_resp, v_ad) )*dx

    solve(F_ad - dJdu ==0, adj, bc)
    """Descomentar para fazer de forma automatica
